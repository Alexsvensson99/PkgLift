...
- Set the 0.10.0 plan version. Regenerate saved 0.9.0 plans before
...
- Add the opt-in `podspec inspect --source-selection flat-swift-globs`
  ...
  in directory and filename components without adding other glob syntax. The
  default command and existing inspection API retain the v1 path grammar, JSON
  and text contract.
...
- Set the source version to `0.7.1`. Regenerate saved migration plans after
  upgrading; plans remain bound to their creating version.
