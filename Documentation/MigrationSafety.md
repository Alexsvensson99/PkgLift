...
Older schema-1 plans remain decodable, but legacy target arrays are treated only as partial context. An AUTO entry without current provenance or consumer-language evidence is refused and must be regenerated before migration. After upgrading, regenerate existing executable plans with the current PkgLift version because preflight binds a plan to the version that created it.
