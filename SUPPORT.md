...
Security fixes target the current `0.4.x` minor line. Older minor lines may be asked to upgrade before a defect is investigated. Source builds from unreviewed branches and modified registry entries are supported only when the report includes enough reproducible evidence to distinguish local changes from a PkgLift defect.
